...
class JHMDB2G(data.Dataset):
    def __init__(self, data_dir='data', text_file='test.txt', transform=None):
        super(JHMDB2G, self).__init__()
        # load split file
        ...
        self.img_dir = 'images_raw'
        self.transform = transform

    def load_img(self, index):
        info = self.indexlist[index]
        act, vid, im_name = info.split('/')[-3:]

        img_path = os.path.join(self.data_dir, self.img_dir, act, vid, im_name.split('_')[1])
        img = Image.open(img_path).convert('RGB')
        w, h = img.size

        annot_path = os.path.join(self.data_dir, 'face_bbox/all', act, vid, im_name.strip('.png')+'.npz')
...
def main():
    ...
    g_data = JHMDB2G(data_dir='/home/SSD5/jason-data/Privacy/dataset/JHMDB',
            text_file='test.txt',
    ...
    for i, data in enumerate(dataloader):
        ...
        print(im.size(), bbox.size(), flag.size(), idx)
